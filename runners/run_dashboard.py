...
Executa, em ordem:
  1. ETL  — extrai dados do Flashscore e persiste no banco
...
def run_etl(engine, Session) -> int:
    logger.info("=" * 60)
    logger.info("ETAPA 1 - ETL: Extracao Flashscore")
    logger.info("=" * 60)
...
def run_training(df: pd.DataFrame, n_splits: int = 5):
    ...
    models = {
        ...
        "Random Forest": Pipeline(
            [
                ("scaler", StandardScaler()),
                ("clf", RandomForestClassifier(n_estimators=300, max_depth=6, min_samples_leaf=10, class_weight="balanced", random_state=42, n_jobs=-1)),
            ]
        ),
        "Calibrated LR": Pipeline(
            [
                ("scaler", StandardScaler()),
                ("clf", CalibratedClassifierCV(estimator=LogisticRegression(solver="lbfgs", max_iter=1000, C=1.0, random_state=42), method="sigmoid", cv=3)),
            ]
        ),
    }
    ...
    for name, pipe in models.items():
        ...
        for fold_i, (train_idx, val_idx) in enumerate(tss.split(X), 1):
            ...
            folds.append({"fold": fold_i, "n_train": len(train_idx), "log_loss": ll, "brier": bs, "accuracy": acc})

            last_probs, last_true = y_prob, y_val
            logger.info(f"  [{name}] Fold {fold_i} | Treino {len(train_idx):4d} -> Val {len(val_idx):4d} | LL={ll:.4f} BS={bs:.4f} Acc={acc:.3f}")
...
def build_dashboard(df_raw: pd.DataFrame, ml_results: dict, calibration_data: dict, df_ev: pd.DataFrame, is_live_data: bool):
    ...
    gs = gridspec.GridSpec(
        5,
        4,  # Expandido para 4 colunas (3 modelos + 1 baseline)
        figure=fig,
        hspace=0.55,
        wspace=0.38,
        top=0.94,
    ...
    # -- Titulo principal ------------------------------------------------------
    fig.text(0.5, 0.97, "EPL xG Prediction Dashboard", ha="center", va="top", fontsize=22, fontweight="bold", color=CLR["text"])
    fig.text(0.5, 0.955, "Pipeline: ETL -> Feature Engineering -> ML (TimeSeriesSplit) -> Expected Value", ha="center", va="top", fontsize=11, color=CLR["subtext"])

    import datetime

    timestamp_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    fig.text(0.97, 0.02, f"Gerado em: {timestamp_str}", ha="right", va="bottom", fontsize=9, color=CLR["subtext"], style="italic")
    ...
    for col_i, kpi in enumerate(kpi_specs):
        ...
        ax.text(0.5, 0.82, kpi["title"], transform=ax.transAxes, ha="center", fontsize=12, fontweight="bold", color=kpi["color"])
        ...
        for j, (label, val, color) in enumerate(metrics):
            y_pos = 0.52 - j * 0.22
            ax.text(0.28, y_pos, label + ":", transform=ax.transAxes, ha="right", fontsize=9, color=CLR["subtext"])
            ax.text(0.32, y_pos, val, transform=ax.transAxes, ha="left", fontsize=13, fontweight="bold", color=color)
    ...
    for spec_gs, metric, title, color, baseline, note in metric_plots:
        ...
        for idx, (name, df_m) in enumerate(ml_results.items()):
            ...
            bars = ax.bar(x + offset, df_m[metric], width, color=model_colors[idx], alpha=0.85, label=name, zorder=3)

        ax.axhline(baseline, color=CLR["subtext"], linestyle="--", linewidth=1.4, label=f"Baseline: {baseline:.3f}", zorder=2)

        ax.set_xlabel("Fold", fontsize=9)
        ax.set_xticks(x)
        ax.legend(fontsize=8)
        ax.set_facecolor(CLR["panel"])
        ax.text(0.98, 0.96, note, transform=ax.transAxes, ha="right", va="top", fontsize=8, color=CLR["subtext"], style="italic")
    ...
    if len(ars_ts) > 5:
        ax_ewma.plot(ars_ts["date"], ars_ts["xg_pro"], color=CLR["green"], linewidth=2, label="Ataque (xG produzido)", zorder=3)
        ax_ewma.fill_between(ars_ts["date"], ars_ts["xg_pro"], alpha=0.15, color=CLR["green"])
        ax_ewma.plot(ars_ts["date"], ars_ts["xg_con"], color=CLR["red"], linewidth=2, label="Defesa (xG concedido)", zorder=3)
        ax_ewma.fill_between(ars_ts["date"], ars_ts["xg_con"], alpha=0.15, color=CLR["red"])
        ax_ewma.axhline(ars_ts["xg_pro"].mean(), color=CLR["green"], linestyle=":", linewidth=1, alpha=0.6)
        ax_ewma.axhline(ars_ts["xg_con"].mean(), color=CLR["red"], linestyle=":", linewidth=1, alpha=0.6)
    ...
    ax_cal.plot([0, 1], [0, 1], "k--", linewidth=1.2, label="Calibração perfeita", color=CLR["subtext"])
    ax_cal.grid(True)
    ...
    for idx, (name, (y_true, y_prob)) in enumerate(calibration_data.items()):
        ...
        try:
            ...
            ax_cal.plot(mean_pred, frac_pos, "o-", color=model_colors[idx], linewidth=2, markersize=5, label=name)
        except Exception:
            pass
    ...
    y_all = np.select([df_raw["home_goals"] > df_raw["away_goals"], df_raw["home_goals"] == df_raw["away_goals"], df_raw["home_goals"] < df_raw["away_goals"]], ["H", "D", "A"], default="")
    mask = y_all != ""
    ...
    feat_labels = [f.replace("ewma", "EWMA-").replace("_xg_", " xG ").replace("_home", " 🏠").replace("_away", " ✈") for f in FEATURE_COLS]
    ...
    for bar, val in zip(bars, sorted_coefs):
        ax_fi.text(val + (0.004 if val >= 0 else -0.004), bar.get_y() + bar.get_height() / 2, f"{val:+.3f}", va="center", ha="left" if val >= 0 else "right", fontsize=8, color=CLR["text"])
    ...
    for label, w in zip(col_labels, col_widths):
        ax_ev.text(x_cursor, 0.93, label, transform=ax_ev.transAxes, ha="left", va="top", fontsize=9, fontweight="bold", color=CLR["yellow"])
        x_cursor += w
    ...
    for row_i, (_, row) in enumerate(df_ev.iterrows()):
        ...
        rect = mpatches.FancyBboxPatch(
            ...
            facecolor=bg_color,
            alpha=0.12 if row["is_value"] else 0,
        ...
        for val, color, w in zip(row_vals, text_colors, col_widths):
            ax_ev.text(x_cursor, y_pos, str(val), transform=ax_ev.transAxes, ha="left", va="top", fontsize=8.5, color=color, zorder=2)
            x_cursor += w

    # ── Salva e exibe ──────────────────────────────────────────────────────────
    output_path = "dashboard.png"
    plt.savefig(output_path, dpi=150, bbox_inches="tight", facecolor=CLR["bg"], edgecolor="none")
