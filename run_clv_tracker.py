...
from src.data.models import CLVTracking, create_tables, get_engine
...
def capture_early_value():
    ...
    try:
        ...
        for bet in value_bets:
            ...
            existing = session.query(CLVTracking).filter_by(event_id=bet.event_id, outcome=bet.outcome).first()
...
def verify_closing_lines():
    ...
    try:
        ...
        pending_trackers = session.query(CLVTracking).filter(CLVTracking.pinnacle_closing_odd.is_(None)).all()
        ...
        for t in pending_to_check:
            ...
            if odds_dict and t.outcome in odds_dict:
                ...
                logger.info(
                    f"{status_clr} Pinnacle Closing validada para {t.home_team} vs {t.away_team} [{t.outcome}]: Early: {t.odd_capturada:.2f} | Closing: {closing_odd:.2f} | CLV+: {t.clv_positivo}"
                )
            else:
